...
| `form`       | `id`, `input` (form tool input: `title`, `description?`, `submitLabel?`, `fields[]` — each `{ id, label, type, options?, placeholder?, required?, value? }`, `type` ∈ text \| textarea \| number \| checkbox \| select \| multiselect), `agent?` | main agent composed a typed form (`mcp__ui__form`); **session is paused until replied** |
