...
import { unregisterSession } from "../features/pulse/pulse-control.js";
 ...
 * loop, drop the session as a Pulse delivery target, abort the CLI (this is what finally kills the
 * sub-agents), close the log, and drop it from the registry so a reconnect falls back to
 * disk-resume. Idempotent via the `done` flag — it runs from both the grace timer and runSession's
 * terminal `finally`. @param {LiveSession} ls */
export function teardown(ls) {
  ...
  // Pulse is a process singleton; only a DEAD session stops being a delivery target — a merely
  // detached one keeps receiving beats (its inbox is alive and the turn output buffers for replay).
  if (ls.session.pulseId != null) unregisterSession(ls.session.pulseId);
  ls.inbox.close();
