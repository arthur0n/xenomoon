...
- Fix note (2026-09-02, issue-digest applied): computable-now half measured on live #295 — raw dump 37,479 chars → `--digest` overview 10,751 (−71%); implement-method's hand-rolled jq (28,158) → `--lane analysis` 8,907 (−68%). The first overview cut was only −22%: per-lane caps sized for READING a lane (4–8k) were also used for the "where is this issue" overview — the overview needed its own tighter cap (1,500/lane). Lesson for step 4: name WHICH question a fetch answers (state vs spec) — two questions, two payload sizes, one verb. Single-lane fetches (qa/review) were already ~4k via `| last` jq — no gain there; the win is full dumps + the analysis spec + identical repeats (now denied by `issue-dedup.sh`, keyed per session+agent so a sub-agent's first fetch never collides with the orchestrator's). Two markers, one id: `op=digest lane=<x>` and `op=dedup`.
