...
| `permission_denied` | `toolName`, `agent?`, `target?` (best-effort file path / command of the denied call), `reason?` (SDK `decision_reason_type`, e.g. `asyncAgent`\|`rule`\|`mode`), `background?`                                                                                                                   | a tool call was auto-denied with no interactive prompt (e.g. a headless/background sub-agent the approver can't reach). Logged to the activity stream; a `background` denial also raises a chat banner                                                                                                                                      |
...
| `autonomousMode`    | `payload` (`{ active, goal, intervalMs, startedAt, lastCheckAt, checks, status, report }`)                                                                                                                                                                                                       | Autonomous Mode state — the standing Main Goal + ON/OFF flag behind the header badge. Snapshot: the client replaces its slice wholesale, so the badge can never desync from the server                                                                                                                                                      |
| `pulse`             | `payload` (`{ active, sleeping, intervalMs, scope, startedAt, lastBeatAt, nextBeatAt, beats, flatBeats, found, suppressed, lastError }`)                                                                                                                                                         | Pulse state — the ambient landing sweep behind the LED. Snapshot (same rule as `autonomousMode`). The server-side `seen` map (id → fingerprint, the delta memory) is deliberately NOT sent                                                                                                                                                  |
...
| `autonomous_mode` | `action`: `start` \| `stop`, `goal?`                   | start/stop Autonomous Mode. `start` needs a `goal`; a malformed message is a no-op (it must not flip the auto-allow-every-tool flag)                                                                                                                                                                     |
| `pulse_mode`      | `action`: `start` \| `stop` \| `now`                   | arm/disarm Pulse, or force one beat. Zero-config — there is no goal to author. NEVER touches the autonomous auto-allow flag                                                                                                                                                                              |
...
- **Auto-deny visibility.** A backgrounded (headless) sub-agent's tool call can be auto-denied by the SDK with no interactive prompt (no approver to reach). The server forwards the SDK's `permission_denied` system message as a `permission_denied` UI message — with the best-effort `target` (file path / command) captured from the earlier `tool_use` — so the friction is visible and actionable in the activity log (and, for background, a banner) instead of dying silently as an `is_error` inside a sub-agent transcript. Fix by granting the path: project/library writes are pre-granted by the `allow-project-edits.sh` PreToolUse hook, and case-by-case paths (including `.claude/`) go in `<project>/.xenomoon/write-grants` (one prefix per line). A plugin `permission-mode` does NOT work — the CLI drops escalating modes from plugin trust tiers.
...
## Pulse (the landing sweep)

Pulse answers a failure the request-shaped session loop cannot: work that FINISHED but never
LANDED (unpushed branches, idle PRs, issues waiting on a human) is invisible until someone goes
looking. It is **off by default**, armed per project, and persists in `<project>/.xenomoon/pulse.json`.

- **One timer per server process**, not per connection (`server/features/pulse/pulse-control.js`).
  Every websocket registers as a possible delivery target and the beat goes to the most recently
  active one — so two browser tabs can't each inject the same sweep. Two server _processes_ on one
  project would still race; the per-project port binding is what prevents that.
- **Delta only.** Checks (`pulse-checks.js`) emit items with a stable `id` and a fingerprint (`fp`)
  of just the fields that decide the action. A beat injects a `[pulse · beat N]` turn **only** for
  ids that are new or whose `fp` changed; unchanged state injects nothing at all. Without this the
  same unpushed branch would be re-reported every interval.
- **Flat beats sleep it.** 6 consecutive beats that find nothing → `sleeping`. Any human message
  silently re-arms it (`noteHumanActivity`), so sleep is a cost guard, not an off switch nobody
  remembers to flip back.
- **Checks are async with timeouts** (`execFile`, never `execFileSync`): a sync `gh` on a timer
  would block the event loop — websockets, permission replies and task updates all stall behind it.
  `gh` is optional; without auth or an `origin` remote it degrades to git-only checks plus one note.
- **It is not Autonomous Mode.** No goal, no planning, no dispatching, no code. It never sets
  `session.autonomousActive` (the auto-allow-every-tool flag) and stands down entirely while
  Autonomous is running. New checks are human-approved: `mcp__ui__pulse op:"propose_check"` files a
  question for the user rather than adding one at runtime.
