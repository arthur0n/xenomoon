...
**Problem** — Layer 3 (`verify_render.gd`) samples one frame at startup with no input. Two bugs
shipped undetected: (1) melee hammer view-model rendering over the gun at rest, (2) a hammer-head
face clipping the near plane after a melee swing (V) leaving a solid square filling the screen.
Both only appear during or after a player-triggered action. The startup snapshot is structurally
blind to this class of artifact.

**Transport** — CLI. Load scene, advance physics frames, call a method or synthesize an input
action, wait for tween/animation to settle, sample SubViewport, assert flood threshold. No live
editor state needed — purely stateless batch. MCP not justified.
...
| Method call (`try_melee()`)                           | WORKS — call directly on the instantiated scene node; simpler and more deterministic than synthesizing key events.                          |
...
| SubViewport capture (`sv.get_texture().get_image()`)  | WORKS — proven in `capture_screenshot.gd` at `SubViewportContainer/SubViewport`.                                                            |
...
- `<trigger>` — one of:
  - `method:<node_path>:<method_name>` — calls method on named node (e.g. `method:Player:try_melee`)
...
- `<flood_threshold_pct>` — fail if any single color covers more than N% of SubViewport pixels (e.g. `40`)
...
Flood algorithm: sample full SubViewport image, bucket each pixel to nearest 8-step RGB bin,
...
```
verify_render_action.gd  input-driven render assert — trigger action/method, settle, flood-check SubViewport
```
...
- First-person view-model is inside `Main/SubViewportContainer/SubViewport` — script must traverse to that SubViewport, same as `capture_screenshot.gd`. If rig path changes, update the constant.
