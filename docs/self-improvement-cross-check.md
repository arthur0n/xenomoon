...
| **3 INFLOW** — tested ingestion skills as automatic "rivers"     | Skill-driven ingestion is **mature**: the researcher agent: research-transcript (source-push) + research-skill-gap (demand-pull) + research-tooling, each a tested path with a durable digest, "cutting is the default" = the transcript's _less-is-more_ rule (`plugin/skills/research-transcript/SKILL.md:19-24,50`). BUT every path is **on-demand** (human/orchestrator-triggered), not a recurring river. No `sync-sessions` pipeline.                                                                                                                                                                               | **PARTIAL.** The _skills_ exist and are good; the **recurrence ("rivers") + a session-harvest pipeline** is the gap. Highest-value borrow.                                                                                                                                 |
...
1. **Session-harvest pipeline (Pillar 2+3 — highest value).** A forge-local capability that reads
   ...
   **`/framework-audit` ledger findings** — **NOT auto-applied**; they enter the existing
   ...
   the 6-bucket present-to-human, and the transcript mode's distill → verify → map shape.
