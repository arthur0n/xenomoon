---
description: Run the project's Maestro acceptance flows on a booted device (iOS Simulator or Android emulator) against an already-running app (POC-first, persisted session) — out-of-band of the per-issue chain
...
script against an app **already running on a booted device — iOS Simulator or Android
emulator** (it never boots servers and never rebuilds the app), reuses a persisted
auth session, and returns
...
2. **Preconditions are the human's/orchestrator's job, not the runner's**: a booted
   simulator/emulator with the app installed and (for dev builds) Metro up from THIS checkout,
...
4. **Report** the agent's result: scenario, PASS/FAIL/BLOCKED, the load-bearing
   evidence (assertion or failing step + screenshot), the device, and — on
   failure — the next move.

## Auth (one-time, manual)

Maestro **cannot automate Sign in with Apple / Google** or any real sign-in form:

1. A **human signs in once** in the app on the device; the session persists on the
...
- A **FAIL files a new `/feedback` bug** (with the failing step + screenshot) — it
  ...
  not a feature bug.
- Run `/uat poc` after a batch of fixes ships to the local device build, to confirm the app
