...
- `plugin/` — the framework's **ONE capability tree** (the only one loaded at runtime): agents,
  ...
  `xenomoon:<name>`.
- `domains/<name>/` — the **install-source catalog** (`webapp`, `expoapp`). A **domain is an install-time
  ...
  populated head-start (issue-driven triage → solution → implement pipeline); `expoapp` is a populated
  React Native/Expo pack (one pack, both platforms). `domains/` stays off the upstream-sync
