...
- **Fix targets:** the deterministic replacement the opportunity named — a script, a `plugin/`
  tool entry, a hook, or an MCP-server config. Touching framework OR bound-project code here is
  fine: the domain-separation rule is about the ledgers staying separate, not a ban on token
  fixes editing code.
...
3. **Capture BEFORE.** Pick the measure that this fix changes and snapshot it now:
   ...
   - **Forward-looking fix** (the effect only shows in FUTURE sessions — e.g. deduping an MCP docs
...
4. **Apply the deterministic replacement.** Make the change the opportunity named — script / tool
...
5. **Verify the change is sound.** If a framework file changed: `rtk npm run validate` (tsc + eslint,
   zero warnings) and `rtk npx prettier --write` the touched files. If a script/hook: exercise it
...
- Token fixes are recorded ONLY in `.claude/token-audits/` (history.json + LEDGER.md). Keep
  self-improvement loops linked by REFERENCE, not by routing.
- If applying this fix reveals a framework-quality issue (a convention, an agent/skill defect), hand
  it to the human in the report — this repo has no framework-audit ledger yet; that stays a manual
  hand-off.
