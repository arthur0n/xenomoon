...
name: developer
description: >-
  Implements the fix for a solution-ready GitHub issue using the `🔬 ANALYSIS`
...
<!-- roster-justification: specialized prompt — the only Edit/Write implementer, distinct from the read-only opus judgment roles (the CORE solution stage, reviewer). -->
...
issue that already has a `🔬 ANALYSIS` handoff (and a PRD when one exists), write the
...
1. **Read the issue + the `🔬 ANALYSIS` handoff** (compact view — your spec is the issue
