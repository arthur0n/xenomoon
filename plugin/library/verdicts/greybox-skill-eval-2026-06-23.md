...
**ADOPT** a new `godot-greybox` skill, SPLIT from `level-designer` per Hermes rec. level-designer stays concept-first brief; godot-greybox = spatial-craft layer that emits/edits an `ArenaLayout` `.tres`, instantiates it via one builder node, and runs a checkable self-audit. Aligns with the SYSTEMS + DATA-DRIVEN directive (layout is DATA the builder reads — mirrors `cast-system` / `godot-enemy-archetype`). All numeric caps are INFERRED → ship checker in REPORT + baseline-diff mode first, harden to pass/fail only after calibrating on Arena + one good + one bad variant. Same discipline as the unverified-regex CI footgun.

## Grounded against THIS repo (Hermes could not read it)

Confirmed `levels/arena.tscn`, `levels/wave_manager.gd`, `levels/arena.gd`, `project.godot`.

1. **Unit scale = 1 Godot unit = 1 m — CONFIRMED.** arena floor slabs 0.2 thick, walls h=4, player capsule ~1.8 m, doorway-scale cover 4–6 m. P8 metrics-zoo dims apply AS-IS (wall 150–200% figure; halls ≥2.0 m; doorways ≥1.25×2.5). No scale conversion needed. `window/stretch/scale_mode="integer"` is render-only, not world scale.

2. **Arena IS the failure case, quantified.**
...
3. **Spawn-to-engagement uses REAL markers.** 24 `SpawnMarker*` Marker3D + `CenterWP` (36,0,24) + 3 `EnemyWP*`. SpawnManager resolves `spawn_marker_paths`. P7 metric = path SpawnMarker→CenterWP (or nearest cover), NOT abstract center. Markers are perimeter → straight-line ~24 m → at run speed too long AND too exposed (no cover between).

4. **Cover-off-navmesh + walkable checks REUSE SpawnManager's exact APIs — do NOT fork.**
   - Nav region found via `get_tree().get_first_node_in_group("nav_region")` (NavFloor in arena has `groups=["nav_region"]`). Map RID via `nav_region.get_navigation_map()`.
   ...
   - LOS / sightline: `PhysicsDirectSpaceState3D.intersect_ray` with `collision_mask = 1` (WALL_MASK) and `EYE_HEIGHT = 1.0` — mirror SpawnManager's `_ray_query` exactly so checker sees the SAME geometry enemies/spawns do.
   - Checker MUST bake/use the SAME navmesh (`level_navmesh.tres`) the `NavigationAgent3D` enemies use. Confirmed single shared region.

5. **FallZone / trap-floor already exists — `fall_zones:Array[AABB]` reuses it, does NOT fork.** `levels/arena.gd`: FallZone Area3D `collision_mask=2`, `body_entered → _on_fall_zone_entered → apply_damage + teleport`; 4 dwell-trap tiles (TrapA–D Area3D sensors). ArenaLayout `fall_zones` = data that the builder instantiates into the existing FallZone Area3D pattern + dwell tiles. Schema describes what's built; no new runtime system.
...
| P2 interior foothold per region                     | ADOPT                             | arena fails it exactly (cover all perimeter)                            |
...
| P5 verticality = restraint (bounded)                | ADOPT                             | arena flat → 0; cap "2–3 floor levels" CALIBRATE                        |
| P6 ≥3 landmarks/regions w/ unique massing           | ADOPT                             | arena = 1 region                                                        |
...
| Metric set (scene-graph / navmesh / raycast)        | ADOPT                             | reuses SpawnManager APIs verbatim                                       |
...
cover density m²/enemy (start REPORT; arena=115, target band 25–40 by playtest) · longest-sightline cap · %-walkable-visible-from-worst-point cap · cover-class ratio (≤~70% single class) · verticality bound (2–3 levels) · spawn-to-engagement band (arena-survival 3–8 s INFERRED) · open:choke floor ratio. Calibrate on Arena + 1 good + 1 bad variant BEFORE any hard gate.
