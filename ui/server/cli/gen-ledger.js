...
// Per-dimension hue for the color chips (D1..D11 around the wheel). FORK DELTA: D10/D11 are
// xenomoon dimensions (abstraction-level/domain-layering; research-loop wiring) — without a hue
// they'd fall back to 0 and read as D1.
...
/** FORK DELTA: `pattern` — the one-line positive exemplar this fork's finding schema defines
 * (state the target behavior, not only the prohibition). A view that silently drops it loses
 * the half of the finding that says what GOOD looks like.
 * @typedef {{ id: string, dim: string, bucket: number, verdict: string, status: string, finding: string, pattern?: string }} Finding */
...
for (const b of BUCKETS) {
  ...
  md.push("");
  for (const f of rows) {
    md.push(`- **${f.id}** · \`${f.dim}\` · _${f.status}_ — ${f.finding}`);
    if (f.pattern) md.push(`  - _pattern:_ ${f.pattern}`);
  }
}
...
const card = (/** @type {Finding} */ f) =>
  `<article class="f v-${esc(f.verdict)}">
     ...
     <p>${esc(f.finding)}</p>
     ${f.pattern ? `<p class="pat"><span>pattern</span> ${esc(f.pattern)}</p>` : ""}
   </article>`;
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .f .pat { margin-top:7px; padding-top:6px; border-top:1px dashed #2a3446; color:#9fd8b4; font-size:12px; }
  .f .pat span { display:inline-block; font-size:10px; text-transform:uppercase; letter-spacing:.05em; color:#6f8f7d; margin-right:6px; }
  .empty { color:#6b7686; }
